function weights = rbfga_weights_BS2(op,ep,xi,xc,par,opts)
        ...
        switch op{i}
            ...
            case 'BS1'
                ...
                rhs(rows,i) = ...
                    -rat*(exp(2*k)/ep^(2*k)*...
                    (B*(Gk(0,k)*ones(nb,1))))...
                    +rat*xc*((1/h)*2*exp(2*k)/ep^(2*(k-1))*...
                    (B*(xi(1:nb,1).*Gk(0,k-1))))...
                    +0.5*sig^2*xc^2*((1/h^2)*2*exp(2*k)/ep^(2*(k-1))*...
                    (B*(-Gk(0,k)*ones(nb,1) + ...
                    2*ep^2*xi(1:nb,1).^2.*Gk(0,k-2))));
                
            case 'BS2'
                rat=par{1};
                sig=par{2};
                rhs(rows,i) = -rat*((exp(2*k)/ep^(2*k)*...
                    (B*(Gk(0,k)*ones(nb,1)))))...
                    +rat*xc(1)*(((1/h)*2*exp(2*k)/ep^(2*(k-1))*...
                    (B*(xi(1:nb,1).*Gk(0,k-1)))))...
                    +rat*xc(2)*(((1/h)*2*exp(2*k)/ep^(2*(k-1))*...
                    (B*(xi(1:nb,2).*Gk(0,k-1)))))...
                    +0.5*sig(1,1)*xc(1)^2*(((1/h^2)*2*exp(2*k)/ep^(2*(k-1))*...
                    (B*(-Gk(0,k)*ones(nb,1) + ...
                    2*ep^2*xi(1:nb,1).^2.*Gk(0,k-2)))))...
                    +0.5*sig(2,2)*xc(2)^2*(((1/h^2)*2*exp(2*k)/ep^(2*(k-1))*...
                    (B*(-Gk(0,k)*ones(nb,1) + ...
                    2*ep^2*xi(1:nb,2).^2.*Gk(0,k-2)))))...
                    +sig(1,2)*xc(1)*xc(2)*(((1/h^2)*4*exp(2*k)/ep^(2*(k-2))*...
                    (B*(xi(1:nb,1).*xi(1:nb,2).*Gk(0,k-2)))));
            case 'spread'
                rat=par{1};
                sig1=par{2}; sig2=par{3};
                rho=par{4};
                ...
                rhs(rows,i) = -rat*((exp(2*k)/ep^(2*k)*...
                    ...
                    +rat*xc(2)*(((1/h)*2*exp(2*k)/ep^(2*(k-1))*...
                    (B*(xi(1:nb,2).*Gk(0,k-1)))))...
                    +0.5*sig1^2*xc(1)^2*(((1/h^2)*2*exp(2*k)/ep^(2*(k-1))*...
                    (B*(-Gk(0,k)*ones(nb,1) + ...
                    2*ep^2*xi(1:nb,1).^2.*Gk(0,k-2)))))...
                    +0.5*sig2^2*xc(2)^2*(((1/h^2)*2*exp(2*k)/ep^(2*(k-1))*...
                    (B*(-Gk(0,k)*ones(nb,1) + ...
                    2*ep^2*xi(1:nb,2).^2.*Gk(0,k-2)))))...
                    +rho*sig1*sig2*xc(1)*xc(2)*(((1/h^2)*4*exp(2*k)/ep^(2*(k-2))*...
                    (B*(xi(1:nb,1).*xi(1:nb,2).*Gk(0,k-2)))));
            case 'xx0'
                rat=par{1};
                sig1=par{2}; sig2=par{3};
