...
elseif nprime(1)=='1'
    ...
    %phi = epsil.^2.*sq(r(:,:,dim))./sqrt(tmp);
    
elseif nprime(1)=='2'
    phi = epsil.^2./sqrt(tmp) - epsil.^4.*sq(r(:,:,dim+1)).^2.*tmp.^-1.5;
    %  phi = epsil.^2./sqrt(tmp) - epsil.^4.*sq(r(:,:,dim)).^2.*tmp.^-1.5;
elseif nprime(1)=='3'
    phi = -3*epsil.^4.*sq(r(:,:,dim+1)).*tmp.^-1.5 + 3*epsil.^6.*sq(r(:,:,dim+1)).^3.*tmp.^-2.5;
    
elseif nprime(1)=='4'
    phi = -3*epsil.^4.*tmp.^-1.5 +18*epsil.^6.*sq(r(:,:,dim+1)).^2.*tmp.^-2.5 - 15*epsil.^8.*sq(r(:,:,dim+1)).^4.*tmp.^-3.5;
    
elseif nprime(1)=='L' & length(nprime)==1
    phi = nd*epsil.^2.*tmp.^-0.5 - epsil.^4.*sq(r(:,:,1)).^2.*tmp.^-1.5;
    
elseif nprime(1:2)=='L2'
    phi = -nd*(nd+2)*epsil.^4.*tmp.^-1.5 + 6*(nd+2)*epsil.^6.*sq(r(:,:,1)).^2.*tmp.^-2.5 - 15*epsil.^8.*sq(r(:,:,1)).^4.*tmp.^-3.5;
